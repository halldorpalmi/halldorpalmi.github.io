...
window.onload = function init() {
    ...
    canvas.addEventListener("mousedown", function(e){
        ...
        var p1 = vec2(center[0], center[1] + size);
        var p2 = vec2(center[0] - size, center[1] - size);
        var p3 = vec2(center[0] + size, center[1] - size);

        // Add new point behind the others
        gl.bufferSubData(gl.ARRAY_BUFFER, 8 * index, flatten(p1));
        gl.bufferSubData(gl.ARRAY_BUFFER, 8 * (index + 1), flatten(p2));
        gl.bufferSubData(gl.ARRAY_BUFFER, 8 * (index + 2), flatten(p3));
